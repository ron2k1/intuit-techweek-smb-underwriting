...
| Steven | LightGBM no-prior-score PD + guarded hazard/recovery NPV policy | brief-faithful cash-flow formula / - | 8,477 (63.7%) | 2,576 (44.5%) | $14.88M | 2,012 | $3.87M | $7.41M | $2.76M | 0.744 | funds reject region with margin guard; drops prior score |

## What This Shows

The biggest spread is not model AUC. It is policy/economics: whether the branch catches amortization/LGD, and whether it chooses to fund the prior-declined region where there are no observed outcomes. Steven's active stack is now LightGBM/no-prior-score PD, weekly hazard timing, recovery regression, and an expected-NPV decision policy with a stricter prior-declined margin guard.
...
- Steven's current branch is the active project policy: LightGBM/no-prior-score PD, brief-faithful NPV, weekly timing/recovery models, and a prior-declined margin guard. It now has the highest labeled-validation NPV in this comparison while reducing unverified reject-region exposure versus the unguarded LightGBM policy.
